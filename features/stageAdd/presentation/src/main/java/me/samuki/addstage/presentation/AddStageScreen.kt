...
import me.samuki.stageadd.presentation.urlPreview.UrlPreview
import me.samuki.stageadd.presentation.urlPreview.UrlPreviewState
...
@Composable
fun StageAddScreen(stageAddNavigation: StageAddNavigation, goBackNavigation: GoBackNavigation) {
    val viewModel: StageAddViewModel = hiltViewModel()

    LaunchedEffect(Unit) {
        viewModel.initState(stageAddNavigation.getJourneyIdArgument())
    }

    viewModel.viewEvent {
        when (it) {
            is StageAddViewModel.ViewEvent.StageCreated -> {
...
@Composable
private fun StageAddContent(
    viewState: StageAddViewModel.ViewState,
    goBack: () -> Unit,
